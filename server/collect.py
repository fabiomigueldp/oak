...
while True:
    try:
        ...
        data={'updated':time.time(),'online':bool(match),'players':players,'maxPlayers':int(match[2]) if match else 12,'version':'26.3-rc-2','chat':messages[-60:],'disk':{'used':disk.used,'total':disk.total,'free':disk.free},'backup':{'last':backups[-1].stat().st_mtime if backups else None,'count':len(backups),'bytes':sum(p.stat().st_size for p in backups)},'warnings':[]}
