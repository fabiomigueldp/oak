...
final class NetworkRegression {
    static void run(Aviary app,ServerPlayer player)throws Exception{
        ...
        try{
            ...
            var json=new JsonObject();json.addProperty("revision",7);json.add("settings",new Gson().toJsonTree(new AviaryStore.Settings(false,"","",500,2)));json.add("ports",new Gson().toJsonTree(List.of(legacy)));json.add("network",JsonParser.parseString("{\"fieldPickup\":true,\"discoverPublic\":true,\"maxOwnedPerches\":4}"));
            ...
            require(store.network.fieldPickup()&&store.network.discoverPublic()&&store.perches.isEmpty(),"Legacy network defaults changed");
            ...
            var control=new AviaryControl(app);var request=new JsonObject();request.addProperty("action","policy");request.addProperty("revision",store.revision);request.addProperty("fieldPickup",false);request.addProperty("discoverPublic",false);request.addProperty("maxFlights",3);request.addProperty("shortcutDistance",400);
            ...
            long revision=store.revision;request.addProperty("revision",revision);request.addProperty("maxFlights",5);rejected(()->control.execute(request),"Invalid policy limit accepted");require(store.revision==revision&&store.settings.maxFlights()==3,"Rejected policy changed state");
            ...
            var pref=store.preferences(player.getUUID());
            store.preferences(player.getUUID(),new AviaryStore.Preferences(pref.favorites(),pref.quick(),pref.freeCamera(),pref.discovered(),List.of(),port.id(),true));
            for(int i=0;i<7;i++)app.arrived(player,"recent_"+i);
            app.arrived(player,"recent_3");store.forget(player.getUUID());pref=store.preferences(player.getUUID());
            require(pref.recent().equals(List.of("recent_3","recent_6","recent_5","recent_4","recent_2"))&&pref.home().equals(port.id())&&pref.introduced()&&pref.favorites().contains("legacy"),"Recent travel corrupted preferences");
