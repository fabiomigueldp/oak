...
This external command is independent of website deployment. BlueMap currently uses resources for Minecraft `26.3-rc-2`, with a snapshot adapter maintained outside this website repository. See the [rc-2 runtime upgrade record](../docs/minecraft-rc2-upgrade.md) for crossplay validation and recovery. The website applies an early quality profile through `public/map-profile.js`, injected by Nginx before the BlueMap module on both `/map/` and `/map/index.html`. Generated map files are not modified by website deployment.
