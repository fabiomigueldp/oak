...
This external command is independent of website deployment. BlueMap currently uses resources for Minecraft `26.3`, with a snapshot adapter maintained outside this website repository. See the [26.3 release upgrade record](../docs/minecraft-263-upgrade.md) for crossplay validation and recovery. The website applies an early quality profile through `public/map-profile.js`, injected by Nginx before the BlueMap module on both `/map/` and `/map/index.html`. Generated map files are not modified by website deployment.
